...
ini_x=0.075; ini_z=0; ini_theta=pi/2-0.0737;
...
%     phi2=phi1; phi3=phi2; v2=v1; v3=v2;   
    ...
    GAt=tic;
    [U,logposition(i),ddphi1(i),ddphi(i)]=GA_Solver(X, xc, d_theta, abc1,abc2,z_th, .6*(phi1-phi2),loop_t, pre_U, k,lane_width);
%     log_U{i}=U;
    GAt_e=toc(GAt);
    logGAt(i)=GAt_e;
    if logGAt(i)<Ts
