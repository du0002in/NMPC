...
% system('\Stereo\RealRun_PXI_new_Cam\RealRun_PXI_new_Cam_RS232\RealRun_PXI_new_Cam_RS232\Debug\RealRun_PXI_new_Cam_RS232.exe &');
system('D:\Stereo\run_rs232_to_laptop.bat &');
lane_width=3.5;
...
V_cum=0; V_max=20;%6/1/15
% ini_x=110.4; ini_z=138.5; ini_theta=0.3672; %real map, starting point
% ini_x=256.0; ini_z=-158.2; ini_theta=5.23;
% ini_x=176.9; ini_z=-265.9; ini_theta=2.9246;
% ini_x=109.4; ini_z=-314.7; ini_theta=-3.19;
% ini_x=0.5; ini_z=0;ini_theta=pi/2;
% ini_x=-109.8; ini_z=-119; ini_theta=--2/3*pi;
ini_x=0.5; ini_z=0; ini_theta=pi/2; %simulated map, starting point
...
abc2=abc1;z_th=0;
pre_U=zeros(2,15);
[U,logposition(1),~]=GA_Solver(X, ini_xc, ini_d_theta, abc1,abc2, z_th, 0, Ts, pre_U, 0,lane_width); horizon=length(U);
...
S=[v2 v1 V_cum+cumsum(U(1,:))]; %6/1/2015
V_cum=V_cum+U(1,1);  %6/1/2015
...
status=1;i=1;stop_all_flag=1;
while status==1 && stop_all_flag==1 && i<131
    stop_all_flag=get_stop_all_flag;
    st=tic; i=i+1;
    [act_x(i), act_z(i), act_theta(i), act_phi(i), ~,logCart(i), act_v(i)]=GA_getActPose;
...
%     phi2=phi1; phi3=phi2; v2=v1; v3=v2;   
    ...
    [log_refined_flag(i), U]=Refine_U(U,x,z,theta,abc1,abc2,z_th,Ts,L_wheels,lane_width);
%     if logposition(i-1)~=7 && logposition(i)==7
%         U(2,1)=atan(L_wheels*log_k(i))-phi1; U(2,2:end)=0;
%     end
    log_U{i}=U;
    GAt_e=toc(GAt);
    logGAt(i)=GAt_e;
...
%     S=[v2 v1 v1+cumsum(U(1,:))];
   V_cum=V_cum+U(1,1);  %6/1/2015
   if U(1,1)*log_d_v_t(i-1)<0 %if there is change of sign in d_v, scyn V_cum with v1
       V_cum=0.5*(V_cum+v1+U(1,1));
   end
    if V_cum>V_max  %6/1/2015
        V_cum=V_max;  %6/1/2015
    elseif V_cum<1.0 && i>20  %6/1/2015
        V_cum=1.00;  %6/1/2015
    end  %6/1/2015
    S=[v2 v1 V_cum+cumsum(U(1,:))-U(1,1)];
    
%     
%     if abs(V_cum-(v1+U(1,1)))>0.7 && v1>3.0
%         V_cum=v1+U(1,1);
%     end
%     if V_cum<=V_max && U(1,1)>0
%         S=[v2 v1 V_cum+cumsum(U(1,:))-U(1,1)]; %6/1/2015
%     else
%         S=[v2 v1 v1+cumsum(U(1,:))]; %6/1/2015
% %         if v1<V_cum
%             V_cum=v1;
% %         end
%     end
...
% % % % %     V_cum=V_cum+U(1,1);  %6/1/2015
% % % % %     if abs(V_cum-(v1+U(1,1)))>0.4
% % % % %         V_cum=v1;
% % % % %     end
% % % % %     if V_cum>V_max  %6/1/2015
% % % % %         V_cum=V_max;  %6/1/2015
% % % % %     elseif V_cum<0  %6/1/2015
% % % % %         V_cum=0;  %6/1/2015
% % % % %     end  %6/1/2015
% % % % %     if V_cum<=V_max && U(1,1)>0
% % % % %         S=[v2 v1 V_cum+cumsum(U(1,:))-U(1,1)]; %6/1/2015
...
%     V_t=(S(3:end)*deno_v+S(1:end-2)-(2+2.064*Ts)*S(2:end-1))/(7.82*Ts*Ts);
    ...
    log_Phi2(i)=Phi(2);
    log_Phi3(i)=Phi(3);
    % the term +U(2,:) is to compensate for the delay
    Phi_t=((Phi(3:end)+0.5*U(2,:))*deno_phi+Phi(1:end-2)-(2+23.15*Ts)*Phi(2:end-1))/(252.3*Ts*Ts);
%     Phi_t=Phi_t+(-.0175/.4*abs(Phi_t)+.0175);
%     Phi_t=Phi(3:end);
    position=min([max([round(logGAt(i)/Ts), 1]),horizon]);
    
    if Phi_t(position)-log_phi_t(i-1)>0.02
        Phi_t(position)=log_phi_t(i-1)+0.02;
    elseif Phi_t(position)-log_phi_t(i-1)<-0.02
        Phi_t(position)=log_phi_t(i-1)-0.02;
    end
