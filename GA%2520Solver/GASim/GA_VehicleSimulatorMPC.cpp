     ...
     while ( flag == 1 ) {
		 ...
		 if ( (fp_manuver = fopen ("D:\\Stereo\\manuver.txt", "r")) != NULL ) {
			 if ( (fscanf (fp_manuver, "%s %lf %s %lf", str1, &v_t, str2, &phi_t)) != 0 ) {
					...
					//12/1/15 memetic the delay abt 0.01s*delay_cycle
					int delay_cycle=6; 
					thetadot = -tan(log_phi[39-delay_cycle]) * v_a0 / L_wheels;
					...
					//10L for the case of dely. If assuming no delay, change 10 to 5
					Sleep(10L);
					
					
					if ( (fp_SMC_log = fopen ("D:\\Stereo\\MPC\\GA Solver\\GASim\\GA_ActualPosition_log.txt","w")) != NULL ) {
						//12/1/15 memetic the delay abt 0.2s
						fprintf(fp_SMC_log, "%6.4f %6.4f %6.4f %6.4f %6.4f %6.4f %6.4f %6.4f %6.4f %6.4f\n", dt, x, z, theta, log_phi[39-delay_cycle], v_a0, new_xc, new_d_theta, phi_t, v_t);
					...
					if ( (fp_log_phi = fopen ("D:\\Stereo\\log_phai_m.txt","w")) != NULL ) {
						//12/1/15 memetic the delay abt 0.2s
						fprintf(fp_log_phi, "%6.4f\n%6.4f\n%6.4f\n",log_phi[(ind2-delay_cycle)<0?0:(ind2-delay_cycle)],log_phi[(ind1-delay_cycle)<0?0:(ind1-delay_cycle)],log_phi[39-delay_cycle]);
