...
%     dumy1tmp(dumy1tmp<connected_pixel*th)=0; dumy1tmp(dumy1tmp>=connected_pixel*th)=1;
    ...
    [sel_xtmpl0,sel_ytmpl0, xtmpl0, ytmpl0,sel_xtmpr0,sel_ytmpr0, xtmpr0, ytmpr0, num_in0, para0]=linefitting_simu2(maxconvimg(:,1:ImgSize2),maxconvimg(:,end-ImgSize2+1:end), D_pre, H, num_ite);
...
pre_abc_right_lane1=conjugate_lane(pre_abc1,[min(pre_World(3,:)) max(pre_World(3,:))],leftrightlane,lane_width);
updateabc_lanewidth(pre_abc_right_lane1,pre_abc_right_lane1,min(pre_World(3,:)),lane_width);
updateLocation(1,0,0,pi/2);
% log_datetime{ii}=datestr(now,'dd-mm-yyyy HH:MM:SS FFF');
log_send_data{ii}={[0 0 pi/2],pre_abc_right_lane1,pre_abc_right_lane1,lane_width};
