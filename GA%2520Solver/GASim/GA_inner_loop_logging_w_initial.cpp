...
*in the MEX function through multiple calls to the MEX function */
 ...
 double x_final, z_final, theta_final, x0, z0, theta0;
...
*MATLAB command window. */
 unsigned __stdcall SecondThreadFunc( void* pArguments ) {
     ...
     while ( flag == 1 ) {
		 ...
		 z = z + zdot * dt; 
		 //Sleep(20L);
		 /*if (( fp_t_m = fopen ("D:\\Stereo\\t_m.txt", "a")) != NULL) {
						fprintf(fp_t_m, "%6.5f\n", dt);
						fclose(fp_t_m);
					}*/
	 }
 ...
 void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
     ...
     char *cmd;
     double *xztheta_final_ptr, *xztheta_ini_ptr;

     cmd = mxArrayToString(prhs[0]);
	 xztheta_ini_ptr=mxGetPr(prhs[1]);
     ...
     if (!strcmp(cmd,"Ini")) {
         ...
         flag = 1;
		 x0=*xztheta_ini_ptr;
		 z0=*(xztheta_ini_ptr+1);
		 theta0=*(xztheta_ini_ptr+2);
