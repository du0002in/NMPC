		...
		*(F_ptr+k)=0;
		...
		{	
			...
			if (j==0) { //make sure the acceleration of phi or steering is within its range
				...
				if (abs(*(d_phi_ptr+ind)-*(d_phi_ptr+(j-1)*P+k))>*max_d_d_phi_ptr) {
					if (*(d_phi_ptr+ind)<0) {
						if ((*max_d_d_phi_ptr+*(d_phi_ptr+(j-1)*P+k))<0)
							*(d_phi_ptr+ind)=(double)rand()/RAND_MAX*2*(*max_d_d_phi_ptr)-*max_d_d_phi_ptr+*(d_phi_ptr+(j-1)*P+k);
						else if ((-*max_d_d_phi_ptr+*(d_phi_ptr+(j-1)*P+k))>0)
							*(d_phi_ptr+ind)=-*max_d_d_phi_ptr+*(d_phi_ptr+(j-1)*P+k);
							//*(d_phi_ptr+ind)=*(d_phi_ptr+ind);
						else *(d_phi_ptr+ind)=-*max_d_d_phi_ptr+*(d_phi_ptr+(j-1)*P+k)+(double)rand()/RAND_MAX*(*max_d_d_phi_ptr-*(d_phi_ptr+(j-1)*P+k));
					}
					else if (*(d_phi_ptr+ind)>0) {
						if ((*max_d_d_phi_ptr+*(d_phi_ptr+(j-1)*P+k))<0)
							*(d_phi_ptr+ind)=*max_d_d_phi_ptr+*(d_phi_ptr+(j-1)*P+k);
							...
							*(d_phi_ptr+ind)=(double)rand()/RAND_MAX*2*(*max_d_d_phi_ptr)-*max_d_d_phi_ptr+*(d_phi_ptr+(j-1)*P+k);
						else *(d_phi_ptr+ind)=(double)rand()/RAND_MAX*(*max_d_d_phi_ptr+*(d_phi_ptr+(j-1)*P+k));
					}
					else {
						if ((*max_d_d_phi_ptr+*(d_phi_ptr+(j-1)*P+k))<=0)
							*(d_phi_ptr+ind)=*max_d_d_phi_ptr+*(d_phi_ptr+(j-1)*P+k);
			...
			if (*(d_phi_ptr+ind)>*max_d_phi_ptr)
				*(d_phi_ptr+ind)=*max_d_phi_ptr;
			else if (*(d_phi_ptr+ind)<-*max_d_phi_ptr)
				*(d_phi_ptr+ind)=-*max_d_phi_ptr;
