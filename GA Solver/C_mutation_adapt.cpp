...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double *max_d_v_acc_ptr, *max_d_phi_ptr,*max_d_v_de_ptr;
	int k;
	srand(time(NULL));
	d_v_ptr=mxGetPr(prhs[0]);
	d_phi_ptr=mxGetPr(prhs[1]);
	pm_adapt_ptr=mxGetPr(prhs[2]);
	max_d_v_acc_ptr=mxGetPr(prhs[3]);
	max_d_phi_ptr=mxGetPr(prhs[4]);
	max_d_v_de_ptr=mxGetPr(prhs[5]);
	...
	for (k=0;k<P;k++)
	{
		for (int j=0; j<horizon; j++)
		{
			...
			{
				double sign_d_v=(*(d_v_ptr+j*P+k)>=0?1:-1);
				if (sign_d_v==1) //acc
					*(d_v_ptr+j*P+k)=(*max_d_v_acc_ptr)*((double)rand()/RAND_MAX);
				else
					*(d_v_ptr+j*P+k)=-(*max_d_v_de_ptr)*((double)rand()/RAND_MAX);

				//*(d_v_ptr+j*P+k)=2*(*max_d_v_acc_ptr)*((double)rand()/RAND_MAX)-(*max_d_v_acc_ptr);
			}
