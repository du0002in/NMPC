	...
	for (k=0;k<P;k++)
	{
		...
		*(F_ptr+k)=0;
		...
		{	
			...
			if (( !tf_safe && v1>v_cut ) || 
				...
				if (*(d_v_ptr+ind)>=0) {
					if (!tf_safe || abs(phi1-phi1_d)>diff_phi_phid)
						*(d_v_ptr+ind)=-(*max_d_v_de_ptr)*0.5*(1+(double)rand()/RAND_MAX)*v1/5.0;
					else
						*(d_v_ptr+ind)=-(*max_d_v_de_ptr)*(double)rand()/RAND_MAX*v1/5.0;
				}
				else {
					if (!tf_safe || abs(phi1-phi1_d)>diff_phi_phid)
						*(d_v_ptr+ind)=-(*max_d_v_de_ptr)*0.5*(1+(double)rand()/RAND_MAX)*v1/5.0;
			...
			if (abs(xc-*std_xc_ptr)<=*xc_td_ptr && abs(d_theta)<0.09 && abs(phi1-phi1_d)<=diff_phi_phid) {
			...
			*(F_ptr+k)=*(F_ptr+k)+w_xc*abs(xc-*std_xc_ptr)+w_d_theta*abs(d_theta)
				...
				//+w_v21*abs(0.5-v1)+w_v*abs(max_v-v1)+w_excess_v*abs(v1-max_v);
		}
