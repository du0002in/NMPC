...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	double *d_v_ptr, *d_phi_ptr, *X0_ptr, *xc_ptr, *d_theta_ptr, 
		...
		*max_d_v_acc_ptr, *max_d_v_de_ptr;
	mwSize P, horizon;
	int k;
	double *F_ptr;
	...
	//21/1/15
	double max_d_d_phi_0=1.5*Ts*Ts, max_d_phi_0=0.02;
	...
	max_d_v_acc_ptr=mxGetPr(prhs[18]);
	max_d_v_de_ptr=mxGetPr(prhs[19]);
		...
		*(F_ptr+k)=0;
		...
		{	
			...
			if (*(d_phi_ptr+ind)>max_d_phi)
				...
				*(d_phi_ptr+ind)=-max_d_phi;
			/*
			double v_cut_xc=0; double v_cut_d_theta=0;
			if (!(abs(xc-*std_xc_ptr)<=*xc_td_ptr && abs(d_theta)<0.09)) {
				v_cut_xc=3.0/0.7*(1-abs(xc-*std_xc_ptr));
				if (v_cut_xc>3.0)
					v_cut_xc=3.0;
				else if (v_cut_xc<1)
					v_cut_xc=1;
				v_cut_d_theta=(3.0-1)/0.11*(0.2-abs(d_theta))+1;
				if (v_cut_d_theta>3.0)
					v_cut_d_theta=3.0;
				else if (v_cut_d_theta<1)
					v_cut_d_theta=1;
			}
			else {
				v_cut_xc=max_v; v_cut_d_theta=max_v;
			}
			double v_cut=(v_cut_xc<v_cut_d_theta)?v_cut_xc:v_cut_d_theta;*/
			double v_cut=1.0;
			bool tf_safe=(abs(xc-*std_xc_ptr)<=(*xc_td_ptr+0.2) && abs(d_theta)<0.09);
			if (( !tf_safe && v1>v_cut ) || 
				(acc>acc0  && v1>v_cut)|| v1>max_v || (abs(phi1-phi1_d)>diff_phi_phid && v1>v_cut)) {
				w_v=0;
				if (*(d_v_ptr+ind)>=0) {
					if (!tf_safe || abs(phi1-phi1_d)>diff_phi_phid)
						*(d_v_ptr+ind)=-(*max_d_v_de_ptr)*(double)rand()/RAND_MAX*((v1-v_cut)/(5.0-v_cut)+
						((abs(d_theta)-0.09)>0?(abs(d_theta)-0.09):0)/0.06 + 
						((abs(xc-*std_xc_ptr)-(*xc_td_ptr+0.2))>0?(abs(xc-*std_xc_ptr)-(*xc_td_ptr+0.2)):0)/0.3)/3.0;
					else
						*(d_v_ptr+ind)=-(*max_d_v_de_ptr)*(double)rand()/RAND_MAX*(v1-v_cut)/(5.0-v_cut);
				}
				else {
					if ((!tf_safe || abs(phi1-phi1_d)>diff_phi_phid) && (double)rand()/RAND_MAX>0.5)
						*(d_v_ptr+ind)=-(*max_d_v_de_ptr)*(double)rand()/RAND_MAX*((v1-v_cut)/(5.0-v_cut)+
						((abs(d_theta)-0.09)>0?(abs(d_theta)-0.09):0)/0.06 + 
						((abs(xc-*std_xc_ptr)-(*xc_td_ptr+0.2))>0?(abs(xc-*std_xc_ptr)-(*xc_td_ptr+0.2)):0)/0.3)/3.0;
				}
				...
				w_v=*(w_ptr+2);
				if (*(d_v_ptr+ind)<0)
					*(d_v_ptr+ind)=(*max_d_v_acc_ptr)*(double)rand()/RAND_MAX;
			}
			...
			if (abs(xc-*std_xc_ptr)<=(*xc_td_ptr+0.5) && abs(d_theta)<=0.15 && abs(phi1-phi1_d)<=diff_phi_phid) {
				max_d_d_phi=max_d_d_phi_0*0.5*(abs(d_theta)/0.15+abs(xc-*std_xc_ptr)/(*xc_td_ptr+0.5));
				max_d_phi=max_d_phi_0*0.5*(abs(d_theta)/0.15+abs(xc-*std_xc_ptr)/(*xc_td_ptr+0.5));
			}
			else {
			...
			double w_acc=(acc>acc0)?w_acc0:0;
			//21/1/2015 change the minimum cruising speed from 1.0 to 0.5;
			//double w_v21=(v1<1.0)?10:0;
			//double w_v21=(v1<0.5)?10:0;
			...
			*(F_ptr+k)=*(F_ptr+k)+w_xc*abs(xc-*std_xc_ptr)+w_d_theta*abs(d_theta)
				+w_acc*acc+(w_d_v)*abs(*(d_v_ptr+ind))+w_d_phi*abs(*(d_phi_ptr+ind))+w_v*abs(max_v-v1);
			
			// L2 norm
			/**(F_ptr+k)=*(F_ptr+k)+w_xc*(xc-*std_xc_ptr)*(xc-*std_xc_ptr)+w_d_theta*(d_theta)*(d_theta)
				+w_acc*acc*acc+(w_d_v)*(*(d_v_ptr+ind))*(*(d_v_ptr+ind))+w_d_phi*(*(d_phi_ptr+ind))*(*(d_phi_ptr+ind))
				+w_v*(max_v-v1)*(max_v-v1);
			*/
		}
		//22/1/15 change v1<max_v to v1<5.0
		if (v1<5.0) v1=5.0;
		thetadot=-tan(phi1)*v1/1.28;
		double tail_cost=0;
		for (int j=0; j<horizon; j++)
		{
			...
			// L2 norm
			//tail_cost=tail_cost+w_xc*(xc-*std_xc_ptr)*(xc-*std_xc_ptr)+w_d_theta*(d_theta)*(d_theta);
		}
