...
*in the MEX function through multiple calls to the MEX function */
 ...
 double *xc, *d_theta, *x00, *z0, *theta0,*phi0, *v0, *dumy;
...
*MATLAB command window. */
 unsigned __stdcall SecondThreadFunc( void* pArguments ) {
	 ...
	 FILE *fp_xc, *fp_d_theta, *fp_manuver, *fp_SMC_log, *fp_x, *fp_z, *fp_theta, *fp_v, *fp_phi;
	 clock_t start_t, end_t;
	 v_t=0; phi_t=0; dt=0; phi_a0=*phi0; phi_a1=*phi0; phi_a2=*phi0;
	 v_a0=*v0; v_a1=*v0; v_a2=*v0;
	 theta = *theta0; x=*x00; z=*z0;
	 a=0; b=tan(*d_theta); c=*xc/cos(*d_theta); L_wheels=1.28;
	 /*for data logging and debugging only*/
	 if ( (fp_SMC_log = fopen ("\\Stereo\\Vehicle Simulator\\ActualPosition_log.txt","w")) != NULL ) {
     ...
     while ( flag == 1 ) {
		 ...
		 if ( (fp_manuver = fopen ("\\Stereo\\manuver.txt", "r")) != NULL ) {
			 if ( (fscanf (fp_manuver, "%s %lf %s %lf", str1, &v_t, str2, &phi_t)) != 0 ) {
					...
					if ((( fp_xc = fopen ("\\Stereo\\Vehicle Simulator\\act_xc.txt", "w")) != NULL) & (( fp_d_theta = fopen ("\\Stereo\\Vehicle Simulator\\act_d_theta.txt", "w")) != NULL )) {
						fprintf(fp_xc, "%4.3f", new_xc);
						fprintf(fp_d_theta, "%4.3f", new_d_theta);
						fclose(fp_xc); fclose(fp_d_theta);
					}

					if ((( fp_v = fopen ("\\Stereo\\Vehicle Simulator\\act_v.txt", "w")) != NULL) & (( fp_phi = fopen ("\\Stereo\\Vehicle Simulator\\act_phi.txt", "w")) != NULL )) {
						fprintf(fp_v, "%4.3f", v_a0);
						fprintf(fp_phi, "%4.3f", phi_a0);
						fclose(fp_v); fclose(fp_phi);
					}

					if ((( fp_v = fopen ("\\Stereo\\v_m.txt", "w")) != NULL) & (( fp_phi = fopen ("\\Stereo\\phai_m.txt", "w")) != NULL )) {
						fprintf(fp_v, "%4.3f", v_a0);
						fprintf(fp_phi, "%4.3f", phi_a0);
						fclose(fp_v); fclose(fp_phi);
					}

					if ((( fp_x = fopen ("\\Stereo\\Vehicle Simulator\\act_x.txt", "w")) != NULL) & (( fp_z = fopen ("\\Stereo\\Vehicle Simulator\\act_z.txt", "w")) != NULL) & (( fp_theta = fopen ("\\Stereo\\Vehicle Simulator\\act_theta.txt", "w")) != NULL )) {
						fprintf(fp_x, "%4.3f", x);
						fprintf(fp_z, "%4.3f", z);
					...
					if ( (fp_SMC_log = fopen ("\\Stereo\\Vehicle Simulator\\ActualPosition_log.txt","a")) != NULL ) {
