...
sendtarget(0,0);
load 'MPC_onlineCtrl';
leftimg_dir='\Stereo\Image Collection6\right';
rightimg_dir='\Stereo\Image Collection6\left';
save_dir='\Stereo\Vehicle Simulator\simu_test1';
global_varibles;
...
step_size_phi=0.18;
ini_xc=-1.8; ini_d_theta=0.086; %These two values need to guarantee that the line is the road center line not the road boundary line
...
%     imwrite(dumy,strcat(save_dir,'\Ridge',int2str(ii),'.jpg'),'jpg');
    ...
    lognum_in(ii)=num_in;
    if num_in<100
        update_w_missinfor_SimulatorMPC;
        continue;
    end
...
%         -sin(Estimate_theta_mean-0.5*pi) cos(Estimate_theta_mean-0.5*pi) -Estimate_z_mean;]*[Particle(1:2,:); ones(1,P_No)];
    ...
    %     inner_thread('Con',Estimate_xc,Estimate_d_theta,pre_abc,L_wheels,Q_s,P_s,phi_d,v_t, xzthetapredict);
