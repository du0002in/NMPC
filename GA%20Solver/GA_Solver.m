...
% st=tic;
acc0=1.6;
v_max=min([sqrt(acc0/abs(k)) sqrt(acc0*1.28/tan(abs(X0(10))))]);
if (v_max>20) %23/2/2016
    v_max=20; %23/2/2016
elseif v_max<1.0
    v_max=1.0;
end
...
% end
% w_d_phi=0;
% if position==7 && abs(X0(10)-phi1_d)<=0.02
if (abs(xc-std_xc)<=(xc_td+0.5) && abs(d_theta)<=0.15) && abs(X0(10)-phi1_d)<=0.05 %Feb 2 2015
    max_d_d_phi=max_d_d_phi*0.5*(abs(d_theta)/0.15+abs((xc-std_xc))/(xc_td+0.5));
    max_d_phi=max_d_phi*0.5*(abs(d_theta)/0.15+abs((xc-std_xc))/(xc_td+0.5));
...
% %     max_d_v=.5*Ts; %m/s
% % end
if position==7 && abs(X0(7))<v_max
    max_d_v_acc=1.5*Ts; max_d_v_de=0.5*Ts;
else
    max_d_v_acc=0.5*Ts; max_d_v_de=2.0*Ts;
end

w=[w_xc w_d_theta w_v w_d_v w_d_phi w_acc];
% w=[w_xc/0.3 w_d_theta/0.09 w_v w_d_v/0.5 w_d_phi/0.02 w_acc];
% d_v=2*max_d_v*rand(P,horizon)-max_d_v; %rand() is btw 0 and 1
d_v=rand(P,horizon); d_v(d_v<0.5)=-1; d_v(d_v>=0.5)=1;
ind=find(d_v==-1); rand_v=max_d_v_de*rand(1,length(ind)); d_v(ind)=-rand_v;
ind=find(d_v==1); rand_v=max_d_v_acc*rand(1,length(ind)); d_v(ind)=rand_v;
...
% logSolvert1=toc(tic);
F=C_Fittness(d_v,d_phi,X0,xc,d_theta,abc1,std_xc,std_v0,w,max_d_d_phi,pre_d_phi,max_d_phi, ...
    xc_td,max_d_d_phi1,v_max,phi1_d,abc2,z_th,max_d_v_acc,max_d_v_de);
[~, I]=max(F);
...
time_td=1.1*Ts;
GAt=tic;
while (i<iteration && (get_status('Con')~=1 || (get_status('Con')==1 && toc(GAt)<1.0*Ts)) ...
        && toc(GAt)<time_td)
    i=i+1;
% for i=1:200
...
%     tic;
    ...
    d_phi=d_phi(ind,:);
%     tt=tt+toc;
    C_mutation_adapt(d_v, d_phi, pm_adapt, max_d_v_acc, max_d_phi,max_d_v_de);
%     tic;
    d_v(P,:)=best_d_v;
    d_phi(P,:)=best_d_phi;
    pm_adapt(P)=best_pm;
