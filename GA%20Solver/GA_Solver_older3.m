function [U, position,max_d_d_phi1,max_d_d_phi]=GA_Solver(X0, xc, d_theta, abc1,abc2,z_th, pre_d_phi, pre_T, pre_U, k, lane_width)
...
tt=0;
i=0;
ND=normrnd(0,1,[P,200]);
expND=exp(-gamma*ND);
time_td=1.6*Ts;
GAt=tic;
while (i<190 && get_status('Con')~=1 && toc(GAt)<time_td)
    i=i+1;
% for i=1:200
